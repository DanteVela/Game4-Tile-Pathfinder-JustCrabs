...
public class Guard : MonoBehaviour
{
    ...
    // Variable for Lose Status
    private Text _LoseStatus;
    ...
    void Start()
    {
        ...
        // Keep GameStatus, if Player dies: Display Lose Status
        _LoseStatus = GameObject.Find("GameStatus").GetComponent<Text>();

        // Disable Text until Player dies
        _LoseStatus.enabled = false;
    }
            ...
            else
            {
                ...
                if (step_length > wpath.magnitude) {
                    ...
                    Vector2 step = wpath.normalized * step_length;
                         if (step.x > 0)
                         {
                              transform.localScale = (Vector3)new Vector2(1.0f, 1.0f);
                         }
                         else if (step.x < 0)
                         {
                              transform.localScale = (Vector3)new Vector2(-1.0f, 1.0f);
    ...
    void OnTriggerEnter2D(Collider2D other)
    {
        ...
        // When Player dies, re-enable text and display _LoseStatus
        _LoseStatus.text = "You Lose! Try Again";
        _LoseStatus.enabled = true;
